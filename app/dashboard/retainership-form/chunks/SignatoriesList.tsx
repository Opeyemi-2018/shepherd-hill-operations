  ...
  const fetchSignatories = async () => {
    ...
    try {
      const signatoryRes = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/operations/retainership/signatory/${formCode}`,
        { headers: { Authorization: `Bearer ${token}` } },
      );
      const signatoryJson = await signatoryRes.json();

      const formRes = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/operations/retainership/form/${formCode}`,
        { headers: { Authorization: `Bearer ${token}` } },
      );
  ...
  const handleSign = async (signatory: Signatory) => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/operations/retainership/signature/update/${formCode}`,
        {
          method: "POST",
          headers: {
  ...
  if (!data || !currentUserSignatory) {
    return (
      <>
        ...
        <div className="text-center py-12 text-red-500 bg-red-50 rounded mt-10">
          You are not authorized to sign this form!!!
        </div>
      </>
    );
  }
