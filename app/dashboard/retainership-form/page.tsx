...
import { SignFormView } from "./chunks/SignatoriesList";
...
export default function RetainershipPage() {
  ...
  useEffect(() => {
    const fetchForms = async () => {
      if (!token) {
        setError("Please log in to access retainership forms");
        setLoading(false);
        return;
      }
      ...
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/api/operations/retainership/list`,
          {
        ...
        if (!res.ok || !json.status) {
          const errorMessage =
            json.message || "Failed to fetch retainership forms";
        ...
        console.error(err);
        const errorMessage = "Network error: Could not load retainership forms";
        setError(errorMessage);
        toast.error(errorMessage);
      } finally {
        setLoading(false);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/operations/retainership/delete/${selectedForm.code}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        },
    ...
    return (
      <div className="min-h-[70vh] flex items-center justify-center p-6 bg-gray-50">
        <div className="max-w-md w-full text-center">
          ...
          <p className="text-gray-700 text-lg leading-relaxed mb-8">{error}</p>
  ...
  return (
    <div className="p-6">
      ...
      {error && (
        <p className="text-red-600 mb-4 bg-white p-2 rounded-md">{error}</p>
      )}
      ...
      <div className="bg-primary-foreground shadow-lg rounded-lg overflow-hidden">
        ...
        {/* Table - Reverted to your original styling */}
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="">
              <tr>
        ...
        {/* Pagination - Your original pagination */}
