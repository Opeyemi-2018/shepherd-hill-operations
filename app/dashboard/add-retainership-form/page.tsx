...
const AddRetainershipForm = () => {
  ...
  const { token } = useAuth();
  const { clientData, loading: clientLoading } = useClient();
  const { staffData, loading: staffLoading } = useStaff();
  const { servicesData, loading: servicesLoading } = useServices();
  ...
  const clientOptions: ComboOption[] =
    clientData?.data?.client_data?.map((c) => ({
      id: String(c.id),
      name: c.name,
    })) ?? [];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        // grade comes from selected service — passed as-is (string id)
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/operations/retainership/generate`,
        {
          method: "POST",
          headers: {
      ...
      router.push("/dashboard/retainership-form");
    } catch (err) {
    ...
    <div className="min-h-screen">
      ...
      <div className="border-b">
        <div className="max-w-4xl mx-auto px-6 py-4 flex items-center gap-2 text-sm text-muted-foreground">
          <ArrowLeft onClick={() => router.back()} className="h-4 w-4" />
